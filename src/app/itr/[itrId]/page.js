...
export default function ItrRedirectPage() {
  ...
  useEffect(() => {
    ...
    async function fetchAndRedirect() {
      try {
        ...
        const currentStep = data.currentStep || 'details';

        const path = getRedirectPath(taxpayerType, currentStep);
        router.replace(path);
      } catch (err) {
  ...
  const getRedirectPath = (filingType, step) => {
    ...
    if (lowerFilingType === 'individual') {
      switch (step) {
        ...
        case 'tax-summary':
          return '/dashboard/tax-summary';
        default:
          return `/dashboard/individual/${step}`;
      }
    } else {
      let stepRoute = step;
      if (step === 'personal-info' || step === 'pan-details') {
        stepRoute = 'details';
      }
