...
export default function LoginPage() {
  ...
  const { login, setTempAuthData, loginWithGoogle, loginWithApple, loginWithMicrosoft } = useAuth();
  ...
  const handleInitialSubmit = (e) => {
    ...
    // Store data temporarily for the confirmation screen
    setTempAuthData({ email, password });
    // Redirect to confirmation page
    router.push(`/confirm?email=${encodeURIComponent(email)}&type=login`);
  };
  ...
  return (
    <div className="w-full min-h-screen h-[1024px] flex justify-center bg-white font-poppins">
      <div className="w-full  flex gap-10">
        ...
        {/* Right Section */}
        <div className="w-[50%] h-full flex flex-col pt-[120px] pb-[40px] px-[40px] gap-[40px] bg-white">
          {/* Top Links */}
          <div className="flex justify-between items-center text-[15px] font-poppins font-normal text-base leading-6 tracking-normal">
            <span >Welcome!</span>
            <span >
              Don't have an account? <Link href="/register" className="font-poppins font-normal text-base leading-6 tracking-normal text-[#3867D6]">Register</Link>
            </span>
          </div>
